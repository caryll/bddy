...
class Context {
	...
	pending(target) {
		...
		if (this.knownTasks.has(targetid)) return;
		this.knownTasks.set(targetid, { state: PENDING, time: new Date })
	}
	fulfill(target, time) {
		const targetid = target + '';
		if (this.knownTasks.has(targetid)
			&& this.knownTasks.get(targetid).state === COMPLETE) return;
		this.knownTasks.set(targetid, { state: COMPLETE, time: time || new Date() });
	}
	for(target) {
		let that = new TargetContext(this.definitions, target);
		that.knownTasks = this.knownTasks;
		return that;
	}
}
...
class TargetContext extends Context {
	...
	async check(...prerequisites) {
		...
		for (let prerequisite of prerequisites) {
			if (this.knownTasks.has(prerequisite + '')) {
				let status = this.knownTasks.get(prerequisite + '');
	...
	async need(...prerequisites) {
		...
		for (let p of prerequisites) {
			let status = this.knownTasks.get(p + '');
